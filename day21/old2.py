...
from itertools import batched, chain, pairwise, permutations, product
...
def move_sequence(keyseq: Iterable[str], keymap_name: Literal["NUMPAD", "DPAD"]) -> list[list[str]]:
    ...
    for key in keyseq:
        ...
        curr = key
    variants = [list(chain(*x)) for x in product(*moveseqs)]
    lowest_variant_score = min(map(sequence_score, variants))
    lvars = [v for v in variants if sequence_score(v) == lowest_variant_score]
    # print(len(lvars), len(variants))
    return lvars


def sequence_score(moveseq: list[str]) -> tuple[int, int]:
